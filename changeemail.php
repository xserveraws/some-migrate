...
<script>
  function emailvalid() {
    var x = document.forms["changeemail"]["newemail"].value;
    var y = document.forms["changeemail"]["repeatnewemail"].value;
    if (x != y) {
        alert("The new emails are not the same!");
        return false;
    }
}
</script>
...
<div class="container">
  ...
  <article class="content">
    ...
    <p>
    <form method="post" action="changeemail.php" name="changeemail" onsubmit="return emailvalid();">
    <table>
    <tr>
    <td>Old email:</td><td> <input type="text" name="oldemail"><br></td>
    </tr>
    <tr>
    <td>New email:</td><td> <input type="text" name="newemail"><br></td>
    </tr>
    <tr>
    <td>Repeat new email:</td><td> <input type="text" name="repeatnewemail"></td>
    </tr>
    </table><br>
    ...
    <?php
      ...
      $queryget = mysqli_query($conn,"SELECT email FROM user WHERE id='$id'") or die("Something went wrong!");
      $row = mysqli_fetch_assoc($queryget);
      $oldemaildb = $row['email'];

      if($oldemaildb == $_POST['oldemail']){
            $querychange = mysqli_query($conn,"UPDATE user SET email='$newemail' WHERE id='$id'");
            echo '<script type="text/javascript">alert("Email changed successfully!");</script>';
      }else{
      echo '<script type="text/javascript">alert("Wrong old email!");</script>';
      }
  }  
    ?>
    </section>
